...
module EmojiTranslate
  ...
  def self.get_all_possibilities(word)
  	...
  	if word.length > 2 && word.end_with?("s")
  		possible_singular = word[0..-2]
  	end
  ...
  def self.find_by_keyword(keyword)
  	...
  	if ["i", "you"].include? keyword
  		return "😊"
  	elsif keyword == "she"
  		return "💁"
  	elsif keyword == "he"
  		return "💁‍♂️"
  	elsif ["we", "they"].include? keyword
  		return "👩‍👩‍👦‍👦"
  	elsif ["am", "is", "are"].include? keyword
  		return "👉"
  	elsif keyword == "and"
  		return "➕"
  	end
  ...
  def self.translate_word(word)
  	...
  	if word.empty?
  		return begin_punctuation + end_punctuation
  	end
